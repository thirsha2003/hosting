...
class Connector_common extends CI_Controller
{
	...
	// End of admin as a cre connector

	public function creater_connector1()
	{
					$method = $_SERVER['REQUEST_METHOD'];
					if($method != 'POST')
					{
						...
						// $result=$query->result_array();
							...
							if($domain_check)
							{
									...
									{     
											...
											$sql = "SELECT * FROM fpa_partners WHERE email='".$params['data']['email']."'";
											...
											$userid = $this->db->insert("fpa_partners", $partnerdata);
											
											}
											else{
												$this->db->where('email', $params['data']['email']);
												$userid = $this->db->update("fpa_partners", $partnerdata);
