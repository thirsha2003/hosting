...
class Adminuserverification extends CI_Controller 
{
			...
			if($check_auth_user == true)
			{
				...
				if ($params['tableName'] == ""){
					...
					$resp = array('status' => 400,'message' =>  'Fields Missing');
							...
							$sql 		= "SELECT * FROM ".$params['tableName']." WHERE mobile = '".$mobile."' || email='".$email_id."'";
							$count 		= $this->db->query($sql)->num_rows();
							$otpvalid 	= 2;
							$eOTP =0;
							$mOTP =0;
	...
	}else
	{
		$check_auth_user	= $this->login->check_auth_user();
			if($check_auth_user == true)
			{
				...
				if($params['key'] != '' )
				{
						$sql = "SELECT * FROM fp_login_history WHERE ".$params['key'] ."&& emailotp_status = 1 && email='".$params['data']['email'] ."'";
						$count = $this->db->query($sql)->num_rows();
					echo $count;
						...
						{	
							...
							return json_output(200, array('status' => 403,'message' => 'Unknown access'));
						}
						}else{
							$resp = array('status' => 201,'message' =>  'Invalid_otp');
							return json_output(200, $resp);
						}
				}else
				{
					$resp = array('status' => 201,'message' =>  'Invalid_otp','data' => '');
					return json_output(403, $resp);
				}

				// $resp = array('status' => 400,'message' =>  'Invalid_otps',);
				// json_output($respStatus,$resp);
			...
			if($check_auth_user == true)
			{
				...
				if($params['key'] != '')
				{
						...
						$email 	= isset($params['data']['email']) ? $params['data']['email'] : null; 
							...
							if($mobile !=null)
							{
								$sql = "SELECT * FROM ".$params['tableName']." WHERE ".$params['key'] ."&& mobotp_status = 1 && email ='".$email."' &&  mobile='".$params['data']['mobile'] ."'";
								$count = $this->db->query($sql)->num_rows();
							}
							else
							{
				...
				$sql = "SELECT * FROM fpa_users WHERE mobile=".$mobile." and email = '".$email."'";
				$respStatus =200;
				$resp = array('status' => 200,'message' =>  'Success','data' => $this->db->query($sql)->row());
				json_output($respStatus,$resp);
